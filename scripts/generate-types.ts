...
function generateDrizzleTypes(): void {
  ...
  try {
    ...
    console.log('✅ Drizzle types generated successfully');
  } catch (error) {
    console.error('❌ Failed to generate Drizzle types:', error);
    process.exit(1);
  }
}
...
async function validateSchemaAlignment(): Promise<void> {
  ...
  if (errors.length > 0) {
    console.error('❌ Schema alignment issues found:');
    errors.forEach(error => console.error(`  - ${error}`));
    process.exit(1);
  }
  
  console.log('✅ Schema alignment validated successfully');
}
...
function updateIndexExports(): void {
  console.log('📦 Updating index exports...');
