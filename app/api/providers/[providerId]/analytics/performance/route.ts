...
import { getPerformanceMetrics, TimePeriod } from "@/db/queries/analytics-queries";
import { z } from "zod";

/**
 * Provider Performance Analytics API
 ...
 * - Response times and on-time rates
 * - Recent reviews and rating trends
 */
...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      const performanceData = await getPerformanceMetrics(providerId, period as TimePeriod);
      ...
      console.error("Analytics error fetching performance metrics:", analyticsError);
      return NextResponse.json(
