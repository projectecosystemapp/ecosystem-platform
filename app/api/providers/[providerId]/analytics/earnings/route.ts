...
import { getProviderEarnings, TimePeriod } from "@/db/queries/analytics-queries";
import { z } from "zod";

/**
 * Provider Earnings Analytics API
 ...
 * 
 * Business Context:
 * - Platform fee is 10% base fee
 * - Guests pay additional 10% surcharge (total 110% of service price)
 * - Providers always receive service price minus 10% base fee
 */
...
export async function GET(
  ...
  try {
    ...
    try {
      // Get earnings data
      const earningsData = await getProviderEarnings(providerId, period as TimePeriod);
      ...
      console.error("Analytics error fetching earnings:", analyticsError);
      return NextResponse.json(
    ...
    console.error("Error in earnings analytics API:", error);
    return NextResponse.json(
